...
from discord import *
from models import Problem, ContestPlayer, Rank
...
class DMOBGame:
    ...
    async def on_finish_submission(self, sub):
        msg = ("{0}, you received a score of {1} for your submission to `{2}`. "
               "Details on your submission have been PM'd to you.")
        await self.bot.send_message(self.channel, msg.format(sub.user.discord_user.mention,
                                    sub.score, sub.problem.code))
    ...
    async def display_problem(self, user, problem_code):
        if Problem(None, problem_code) in self.contest.problems:
            ...
            em = Embed(title="Problems List",
                       description="Problems in the `{}` contest.".format(self.contest.name),
                       color=BOT_COLOUR)
